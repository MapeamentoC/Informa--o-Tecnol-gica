...
with open('./dicionarios\\textos_1.json', 'a', encoding='utf8') as outfile:
                ...
                try:
                    ...
                    if driver.current_url.endswith('.com.br') or driver.current_url.endswith('.com.br/'):
                        ...
                        try:
                            ...
                            try:
                                if valid == 0:
                                    lista = [item.get_attribute('href') for item in driver.find_elements(
                                        'xpath', '//*[contains(a,href)]') if (item.get_attribute('href')) is not None]
                                    for item in lista:
                                        if '.pdf' in item:
                                            memo = item
                                            break
                            ...
                            try:
                                if valid == 0:
                                    if memo == '':
                                        lista = [item.get_attribute('content') for item in driver.find_elements(
                                            'css selector', 'meta') if validators.url(item.get_attribute('content'))]
                                        for item in lista:
                                            if 'pdf' in item:
                                                memo = item
                                                break
